...
extern crate float_pigment_consistent_bincode as bincode;
#[macro_use]
extern crate serde;
...
where
    ...
    O: Options,
{
    let (size, _) = options.serialized_size(&element).unwrap();
...
fn the_same_with_impl<V, W, O>(element: V, rhs: W, options: &mut O)
where
    V: serde::Serialize + serde::de::DeserializeOwned + PartialEq + Debug + 'static,
    W: serde::Serialize + serde::de::DeserializeOwned + PartialEq + Debug + Clone + 'static,
    O: Options,
{
    let (size, _) = options.serialized_size(&element).unwrap();
    ...
    {
        let encoded = options.serialize(&element).unwrap();
        let decoded: W = options.deserialize(&encoded[..]).unwrap();
        let decoded_reader: W = options.deserialize_from(&mut &encoded[..]).unwrap();

        assert_eq!(decoded, rhs);
        assert_eq!(decoded_reader, rhs);
        assert_eq!(size, encoded.len() as u64);
    }
}

fn the_same_with<V, W>(element: V, rhs: W)
where
    V: serde::Serialize + serde::de::DeserializeOwned + PartialEq + Debug + Clone + 'static,
    W: serde::Serialize + serde::de::DeserializeOwned + PartialEq + Debug + Clone + 'static,
{
    ...
    macro_rules! all_endians {
        ($element:expr, $options:expr) => {
            the_same_with_impl(
                $element.clone(),
                rhs.clone(),
                &mut $options.with_native_endian(),
            );
            the_same_with_impl(
                $element.clone(),
                rhs.clone(),
                &mut $options.with_big_endian(),
            );
            the_same_with_impl(
                $element.clone(),
                rhs.clone(),
                &mut $options.with_little_endian(),
            );
        };
    }
...
fn test_serialized_size() {
    ...
    assert!(opt.serialized_size(&0u8).unwrap().0 == 1);
    assert!(opt.serialized_size(&0u16).unwrap().0 == 2);
    assert!(opt.serialized_size(&0u32).unwrap().0 == 4);
    assert!(opt.serialized_size(&0u64).unwrap().0 == 8);

    // length isize stored as u64
    assert!(opt.serialized_size(&"").unwrap().0 == LEN_SIZE);
    assert!(opt.serialized_size(&"a").unwrap().0 == LEN_SIZE + 1);

    assert!(opt.serialized_size(&vec![0u32, 1u32, 2u32]).unwrap().0 == LEN_SIZE + 3 * (4));
}
...
fn test_zero_copy_parse_deserialize_into() {
    ...
    impl<'storage> bincode::io::Read for SliceReader<'storage> {
        #[inline(always)]
    ...
    impl<'storage> BincodeRead<'storage> for SliceReader<'storage> {
        ...
        fn set_barrier(&mut self, _offset: usize) {
            // empty
        }
...
fn test_varint_length_prefixes() {
    let a = [(); 127]; // should be a single byte
    let b = [(); 250]; // also should be a single byte
    let c = [(); 251];
...
#[test]
fn test_cross_version_add_field() {
    #[derive(PartialEq, Eq, Clone, Serialize, Deserialize, Debug)]
    enum OldVersionEnum {
        Unknown,
        A,
        B { s: OldVersionStruct },
    }

    #[derive(PartialEq, Eq, Clone, Serialize, Deserialize, Debug)]
    struct OldVersionStruct {
        a: usize,
    }
    ...
    the_same_with(
        NewVersionEnum::A {
            s: NewVersionStruct { a: 123, b: true },
        },
        OldVersionEnum::A,
    );
    ...
    the_same_with(
        OldVersionEnum::B {
            s: OldVersionStruct { a: 456 },
        },
        NewVersionEnum::B {
            s: NewVersionStruct { a: 456, b: false },
            t: Default::default(),
        },
    );
    ...
    the_same_with(
        NewVersionEnum::B {
            s: NewVersionStruct { a: 456, b: true },
            t: Default::default(),
        },
        OldVersionEnum::B {
            s: OldVersionStruct { a: 456 },
    ...
    the_same_with(NewVersionEnum::C, OldVersionEnum::Unknown);

    the_same_with(OldVersionEnum::Unknown, NewVersionEnum::Unknown);
}
