...
pub struct Serializer<W, O: Options> {
    writer: W,
    sizes_list: alloc::vec::IntoIter<u32>,
    _options: O,
}
...
impl<W: Write, O: Options> Serializer<W, O> {
    ...
    pub fn new(w: W, options: O, sizes_list: Vec<u32>) -> Serializer<W, O> {
        Serializer {
            writer: w,
            sizes_list: sizes_list.into_iter(),
            _options: options,
        }
    }

    fn take_size(&mut self) -> Result<u32> {
        self.sizes_list
            .next()
            .ok_or_else(|| ErrorKind::SizeLimit.into())
    }
...
impl<'a, W: Write, O: Options> serde::Serializer for &'a mut Serializer<W, O> {
    ...
    fn serialize_tuple_variant(
        ...
        let variant_index = variant.parse().unwrap_or(variant_index);
        O::IntEncoding::serialize_u32(self, variant_index)?;
        let s = self.take_size()?;
        O::IntEncoding::serialize_u32(self, s)?;
        Ok(Compound { ser: self })
    }
...
impl<O: Options> SizeChecker<O> {
    ...
    fn begin_sized_segment(&mut self) -> SizedSegment {
        let index = self.sizes_list.len();
        self.sizes_list.push(0);
        let cur_size = self.total;
        SizedSegment { index, cur_size }
    }

    fn end_sized_segment(&mut self, s: SizedSegment) -> Result<()> {
        let SizedSegment { index, cur_size } = s;
        let self_size = self.total - cur_size;
        if self_size > core::u32::MAX as u64 {
            Err(ErrorKind::SizeLimit)?;
        }
        self.sizes_list[index] = self_size as u32;
        self.add_discriminant(self_size as u32)
    }
}
...
impl<'a, O: Options> serde::Serializer for &'a mut SizeChecker<O> {
    ...
    fn serialize_tuple_variant(
        ...
        let sized_segment = Some(self.begin_sized_segment());
        Ok(SizeCompound {
            ser: self,
            sized_segment,
        })
    ...
    fn serialize_unit_variant(
        ...
        let variant_index = variant.parse().unwrap_or(variant_index);
        self.add_discriminant(variant_index)?;
        let ss = self.begin_sized_segment();
        self.end_sized_segment(ss)?;
        Ok(())
    }
...
pub(crate) struct SizeCompound<'a, S: Options + 'a> {
    ...
    sized_segment: Option<SizedSegment>,
}
...
impl<'a, O: Options> serde::ser::SerializeSeq for SizeCompound<'a, O> {
    ...
    fn end(mut self) -> Result<()> {
        if let Some(ss) = self.sized_segment.take() {
            self.ser.end_sized_segment(ss)?;
        }
        Ok(())
    }
}
