...
pub(crate) fn serialize_into<W, T: ?Sized, O>(
    writer: W,
    value: &T,
    mut options: O,
    sizes_list: Vec<u32>,
    ...
    O: InternalOptions,
{
    ...
    let mut serializer = crate::ser::Serializer::<_, O>::new(writer, options, sizes_list);
...
where
    ...
    O: InternalOptions,
{
    let (actual_size, sizes_list) = serialized_size(value, &mut options)?;
    let mut writer = Vec::with_capacity(actual_size as usize);

    serialize_into(&mut writer, value, options.with_no_limit(), sizes_list)?;
    Ok(writer)
}

pub(crate) fn serialized_size<T: ?Sized, O: InternalOptions>(
    value: &T,
    options: O,
) -> Result<(u64, Vec<u32>)>
where
    T: serde::Serialize,
{
    let mut size_counter = crate::ser::SizeChecker {
        options,
        total: 0,
        sizes_list: vec![],
    };
    ...
    result.map(|_| (size_counter.total, size_counter.sizes_list))
}
