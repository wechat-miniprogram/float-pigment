...
//! let my_options = float_pigment_consistent_bincode::DefaultOptions::new();
//! ```
//!
...
/// ```rust
/// use float_pigment_consistent_bincode::Options;
/// let my_options = float_pigment_consistent_bincode::DefaultOptions::new().with_limit(1024);
/// ```
///
...
pub trait Options: InternalOptions + Sized {
    ...
    #[inline(always)]
    fn serialized_size<T: ?Sized + serde::Serialize>(self, t: &T) -> Result<(u64, SizeDetail)> {
        crate::internal::serialized_size(t, self).map(|(s, sizes_list)| (s, SizeDetail(sizes_list)))
    }
    ...
    fn serialize_into<W: Write, T: ?Sized + serde::Serialize>(
        ...
        crate::internal::serialize_into(w, t, self, d.0)
    }
