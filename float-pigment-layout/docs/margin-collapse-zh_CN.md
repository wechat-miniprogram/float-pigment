...
BFC 阻断信号 `isolated` 是**节点固有属性**（display/position/有无父），在 `compute_block_or_inline_series` 开头局部计算，三处复用（start/end/empty 判定）。

**不通过 `ComputeRequest` 全局传播**——保证 BFC 阻断只影响 block flow 路径，flex/grid 布局零波及（它们走自己的 `Flex::compute` / `Grid::compute`，不读 `isolated`）。
...
│                                                                   │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  STEP 1  算 isolated = establishes_bfc(node)                │  │
│  │  当前节点是否建立 BFC（不与子塌陷）                         │  │
│  └──────────────────────────┬──────────────────────────────────┘  │
│                             ▼                                     │
│  ┌─────────────────────────────────────────────────────────────┐  │
...
| # | 审查项 | 标准 | 状态 | 关键证据 |
|---|--------|:---:|:---:|----------|
| 1 | (a) 父-首子 top | border/padding 阻断 + BFC + clearance | ⚠️ 部分 | `is_margin_start_collapsible`：border/padding ✓、isolated ✓；**clearance 缺失**（需 `clear()` accessor） |
...
| 4 | (d) 空块穿透 | 不建 BFC + min:0 + h:0\|auto + 无子 | ✅ | `is_empty_block`：含 `isolated` 短路（BFC 容器不穿透） |
...
| 12 | 同一 BFC 前提 | 跨 BFC 不合并 | ✅ | 父端 `isolated` + 子端 BFC 分支，双向阻断（当前 accessor 覆盖内） |
